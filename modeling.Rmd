...
Excluding arboreal mammals. all camera trap locations: 40 total species.
...
Compared to only CT locations that have LiDAR data: 38 total species
...
obs_table <- m %>%
  ...
  filter(!Species %in% c("Herpestes sp.", "Soricidae", "Tragulus spp.", 
                         "Unid civet", "Unid Rat", "Muntiacus spp.")) %>%
...
terr_traits_trim <- terr_traits[!terr_traits$Species %in% c("Herpestes sp.", "Soricidae", "Tragulus spp.", 
                                             "Unid civet", "Unid Rat", "Muntiacus spp."),
                               -c(2,8,9)]
...
#fd metrics not calculated for CT locations with < 3 species (n = 14)
...
#three locations are missing FD metric data, all are off-trail locations, 2 LS2 and 1 LG
...
obs_table_pt <- obs_table_pt[,-c(1, 35)]
...
How do the traditional community metrics compare to the functional diversity metrics?
...
```{r}
plotcoef(top_shan_model[[1]], intercept = FALSE, title = "Terrestrial Mammal Diversity - Top Mod", ylab = "") +
  theme_classic()
```
...
frich_model <- glm(FRich ~ 
              ...
              data = stand_mets_FD)
#dredge
frich_models <- dredge(frich_model)
...
```{r}
plotcoef(top_frich_model[[1]], intercept = FALSE, title = "Terrestrial Mammal Functional Richness - Top Model", ylab = "") +
  theme_classic()
```
...
feven_model <- glm(FEven ~ 
              ...
              data = stand_mets_FD)
#dredge
feven_models <- dredge(feven_model)
...
```{r}
plotcoef(top_feven_model[[1]], intercept = FALSE, title = "Functional Evenness - Top Model", ylab = "") +
  theme_classic()
```
...
fdiver_model <- glm(FDiver ~ 
              ...
              data = stand_mets_FD)
#dredge
fdiver_models <- dredge(fdiver_model)
...
```{r}
plotcoef(top_fdiver_model[[1]], intercept = FALSE, title = "Functional Divergence - Top Model", ylab = "") +
  theme_classic()
```
...
pt_mets <- stand_mets %>%
  ...
  summarize(across(where(is.numeric), mean, na.rm = TRUE))  # I could add the StdDev of variables too
...
```{r}
plot(rich_models_pt_avg, full = FALSE, intercept = FALSE, main = "Terrestrial Mammal Richness (partition) - Av. Mod")
```
...
```{r}
plot(shan_models_pt_avg, full = FALSE, intercept = FALSE, main = "Terrestrial Mammal Diversity (partition) - Av. Mod")
```
...
```{r}
plotcoef(top_frich_pt_model[[1]], intercept = FALSE, title = "Functional Richness (partition) - Top Mod", ylab = "") +
  theme_classic()
```
...
```{r}
plotcoef(top_feven_pt_model[[1]], intercept = FALSE, title = "Functional Evenness (partition) - Top Mod", ylab = "") +
  theme_classic()
```
...
```{r}
plotcoef(top_fdiver_pt_model[[1]], intercept = FALSE, title = "Functional Divergence (partition) - Top Mod", ylab = "") +
  theme_classic()
```
...
```{r}
#read in community metrics from census data, arboreal mammals only
lt_div_mets_arb <- read.csv(file = "data/census_arb_div_mets.csv", header = TRUE)
lt_div_mets_arb <- lt_div_mets_arb[,-1]

#fix partition names
lt_div_mets_arb <- lt_div_mets_arb %>%
...
#add forest structure metrics
lt_div_mets_arb <- left_join(lt_div_mets_arb, pt_mets)

#removing two montane partitions that lack structure mets
lt_div_mets_arb <- lt_div_mets_arb[!lt_div_mets_arb$partition %in% c("MO.I", "MO.II"),]
```
...
Top model summary output. I have to adjust which predictors are included, since I can only have up to 8 predictors for a model with 11 data points in the outcome variable. No reliable predictors, but some evidence for closed gap space and pts\<2m being positive, and CRR being negative predictors. Top model (not shown) is intercept-only mod.

```{r message=FALSE, warning=FALSE, include=FALSE}
#fitting global model for use in dredge
rm_pt_arb <- glm(richness ~ 
             #max.height.sc +
             #mean.tree.h.sc +
             CRR.rho.sc +
             #sd.r.sc +
           ...
           family = Gamma(link = "log"), 
           offset = log(effort),
           data = lt_div_mets_arb)
#dredge
rich_pt_arb_models <- dredge(rm_pt_arb)

#selecting top model by AIC
top_rich_pt_arb_model <- get.models(rich_pt_arb_models, subset = 1)

summary(top_rich_pt_arb_model[[1]])

#plotcoef(top_rich_pt_arb_model[[1]], intercept = FALSE, title = "Arboreal Mammal Richness (partition) - Top Mod", ylab = "") + theme_classic()
```

averaged model output - averaging top models (delta AIC values \< 6)
I've increased the dAIC to 6 since AIC values varied quite a bit - dAIC < 2 only includes two or three models in some instances

```{r message=FALSE, warning=FALSE, include=FALSE}
#averaging models with dAIC<2
rich_models_pt_arb_avg <- model.avg(rich_pt_arb_models, subset = "delta" < 6)

summary(rich_models_pt_arb_avg)
```

```{r}
plot(rich_models_pt_arb_avg, full = NA, intercept = FALSE, main = "Arboreal Mammal Richness (partition) - Av. Mod")
```

#### Species diversity model

Top model summary output not shown. No reliable predictors, but the ones nearest reliability are CRR (neg), closed gap space (pos), and vFRcanopy (pos)

```{r}
#fitting global model for use in dredge
sm_pt_arb <- glm(shannon ~ 
             #max.height.sc +
             mean.tree.h.sc +
             CRR.rho.sc +
             #sd.r.sc +
           ...
           data = lt_div_mets_arb)
#dredge
shan_pt_arb_models <- dredge(sm_pt_arb)

#selecting top model by AIC
top_shan_pt_arb_model <- get.models(shan_pt_arb_models, subset = 1)

summary(top_shan_pt_arb_model[[1]])

#plotcoef(top_shan_pt_arb_model[[1]], intercept = FALSE, title = "Arboreal Mammal Diversity (partition) - Top Mod", ylab = "") + theme_classic()
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
#averaging models with dAIC<2
shan_models_pt_arb_avg <- model.avg(shan_pt_arb_models, subset = "delta" < 6)

summary(shan_models_pt_arb_avg)
```

```{r}
plot(shan_models_pt_arb_avg, full = NA, intercept = FALSE, main = "Arboreal Mammal Diversity (partition) - Av. Mod")
```

#### Species evenness model

Top model summary output. CRR rho is the only var in the top model (neg), but other vars show up in the avg mod: vert rumple (neg), closed gap space (pos), and mean tree h (neg), being the closest to reliability.

```{r message=FALSE, warning=FALSE, include=FALSE}
#fitting global model for use in dredge
em_pt_arb <- glm(evenness ~ 
             #max.height.sc +
             mean.tree.h.sc +
             CRR.rho.sc +
             #sd.r.sc +
           ...
           data = lt_div_mets_arb)
#dredge
even_pt_arb_models <- dredge(em_pt_arb)

#selecting top model by AIC
top_even_pt_arb_model <- get.models(even_pt_arb_models, subset = 1)

summary(top_even_pt_arb_model[[1]])
```

```{r}
plotcoef(top_even_pt_arb_model[[1]], intercept = FALSE, title = "Arboreal Mammal Evenness (partition) - Top Mod", ylab = "") +
  theme_classic()
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
#averaging models with dAIC<2
even_models_pt_arb_avg <- model.avg(even_pt_arb_models, subset = "delta" < 6)

summary(even_models_pt_arb_avg)
```

```{r}
plot(even_models_pt_arb_avg, full = NA, intercept = FALSE, main = "Arboreal Mammal Evenness (partition) - Av. Mod")
```

## ORDINATION ANALYSIS

Using PCA to see how scan locations group together and to identify which forest structure metrics best differentiate between forest types and partitions. I'm not exactly sure which metrics I will ultimately include. I'm still not sure about including lad.max and vertical rumple. And most of my modeling omits basal area since it's highly correlated with two other, potentially more informative metrics. 
...
```{r}
ct_div_mets_terr %>%
  pivot_longer(cols = c(6:24), names_to = "metric", values_to = "value") %>%
  group_by(metric) %>%
...
### arboreal mammals (census data) by partition

Structure metrics X species richness

```{r}
lt_div_mets_arb %>%
  pivot_longer(cols = c(7:25), names_to = "metric", values_to = "value") %>%
  group_by(metric) %>%
  mutate(cor = cor(value, richness, method = "spearman", use = "complete.obs"),
         subtitle = paste("Spearman's r =", round(cor, 2))) %>%
...
```{r}
divmets <- c("n.all", "shannon_ct", "div_even")
...
for (i in 1:length(fs_mets)) {
  for (j in 1:length(divmets)) {
    cor_matrix[i, j] <- cor(stand_mets[[fs_mets[i]]], stand_mets[[divmets[j]]], method = "pearson")
  }
}
```

```{r}
divmets <- c("n.terr", "Shannon", "evenness")
...
```{r}
divmets <- c("richness", "shannon", "evenness")

cor_matrix3 <- matrix(nrow = length(fs_mets), ncol = length(divmets))
rownames(cor_matrix3) <- paste0(fs_mets, ".sc")
colnames(cor_matrix3) <- divmets

#calculate Pearson correlation coefficients
for (i in 1:length(fs_mets)) {
  for (j in 1:length(divmets)) {
    cor_matrix3[i, j] <- cor(lt_div_mets_arb[[paste0(fs_mets, ".sc")[i]]], lt_div_mets_arb[[divmets[j]]], method = "pearson")
  }
}


kbl(as.data.frame(cbind(cor_matrix, cor_matrix2, cor_matrix3)), 
    format = "html", 
    caption = "Pearson correlations beteen structure metrics and diversity metrics") %>%
  kable_styling(bootstrap_options = "striped") %>%
  add_header_above(c(" " = 1, "Terrestrial by ct" = 3, "Terrestrial by partition" = 3, "Arboreal by partition" = 3))
```
...
```{r}
ct_div_mets_terr$type <- "terrestrial"
lt_div_mets_arb$type <- "arboreal"

ct_div_mets_terr$richness <- ct_div_mets_terr$n.terr
ct_div_mets_terr$shannon <- ct_div_mets_terr$Shannon

all_div_mets <- bind_rows(ct_div_mets_terr[,c("partition","richness","shannon","evenness","type")], 
                          lt_div_mets_arb[,c("partition","richness","shannon","evenness","type")])

long_all_div_mets <- all_div_mets %>%
  pivot_longer(cols = c(richness, shannon, evenness), names_to = "Metric", values_to = "Value")

long_all_div_mets$partition <- factor(long_all_div_mets$partition, levels = c("PS1", "FS1", "AB1", "AB2",
                                          "LS1", "LS2","LG1","LG2", "UG1","UG2", "MO1", "MO2"),
                    ordered = TRUE)
```

comparing three community metrics by partition - arboreal (census data) vs terrestrial (ct data) mammals

```{r fig.height=4, fig.width=10, message=FALSE, warning=FALSE}
p1 <- long_all_div_mets %>%
  filter(Metric == "richness") %>%
  ggplot(aes(x = partition, y = ifelse(type == "arboreal", -Value, Value), fill = type)) +
...
p2 <- long_all_div_mets %>%
  ...
  labs(title = "Shannon Diversity",
       y = "Shannon Diversity Index", x = "")

p3 <- long_all_div_mets %>%
...
cowplot::plot_grid(p1, p2, p3, nrow = 1, rel_widths = c(.3, .3, .4))
```

Create table comparing correlation coefficients for each metric (row) and each diversity metric (col)