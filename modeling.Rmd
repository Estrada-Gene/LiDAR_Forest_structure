...
cmp1 <- ggplot(div_mets[div_mets$n.all > 0,], aes(x = habitat, y = n.all)) +
...
cmp2 <- ggplot(div_mets[div_mets$shannon_ct > 0,], aes(x = habitat, y = shannon_ct)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) +
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
...
cmp3 <- ggplot(div_mets[div_mets$div_even > 0,], aes(x = habitat, y = div_even)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) +
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
...
#plotting - only the functional models for now
ggplot(coef_df, aes(x = estimate, y = predictor, color = model)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(xmin = `2.5 %`, xmax = `97.5 %`), width = 0, 
                position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(xmin = `25 %`, xmax = `75 %`), width = 0, 
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  theme_classic() +
  labs(title = "Species Richness Predictors at Different Scales",
       x = "Coefficient Estimate", y = "") +
  theme(axis.text.y = element_text(hjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(reverse = TRUE)) +
...
ggplot(coef_df, aes(x = estimate, y = factor(predictor, levels = rev(metrics_order)), color = model)) +
  ...
  theme_classic() +
  labs(title = "Shannon Diversity Predictors at Different Scales",
       x = "Coefficient Estimate", y = "", color = "model scale:") +
  theme(axis.text.y = element_text(hjust = 1), legend.position = "bottom",
        legend.text = element_text(size = 9)) +
  guides(color = guide_legend(reverse = TRUE)) +
  xlim(-2.8, 2.4) +
  annotate("text", x = 1.9, y = 12, label = "opposite direction\n of effects\n (eco. scales\n vs. grid scales)", color = "grey30", size = 3) +
  annotate("text", x = 1.9, y = 6.5, label = "same direction\n of effects", color = "grey30", size = 3)  +
  annotate("text", x = 1.9, y = 2.5, label = "mixed direction\n of effects", color = "grey30", size = 3) +
  scale_color_discrete(labels = rev(c("eco: camera trap", "eco: partition",
                                      expression("grid: 0.75 km"^2), 
                                      expression("grid: 1.7 km"^2)))) +
  scale_y_discrete(labels = c("mean.tree.h.sc" = "tree h",
                              "rumple.sc" = "rumple",
                              "zentropy.sc" = "vert. pt. dist.",
                              "lad.max.sc" = "LAD",
                              "CRR.rho.sc" = "CRR",
                              "ClosedGapSpace.sc" = "gap vol.",
                              "vFRcanopy.sc" = "veg. vol. ratio",
                              "sd.r.sc" = "tree h sd",
                              "max.height.sc" = "tree h max.",
                              "stem.vol.sc" = "tree vol.",
                              "mean.dbh.sc" = "tree dbh",
                              "vzrumple.sc" = "vert. pt. complex.",
                              "stand.dens.sc" = "tree density",
...
ggplot(coef_df, aes(x = estimate, y = predictor, color = model)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  theme_classic() +
  labs(title = "Species Evenness Predictors at Different Scales",
       x = "Coefficient Estimate", y = "") +
  theme(axis.text.y = element_text(hjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(reverse = TRUE)) +
...
ggplot(aes(x = estimate, y = predictor, color = model)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  theme_classic() +
  labs(title = "Func. Richness Predictors at Different Scales",
       x = "Coefficient Estimate", y = "") +
  theme(axis.text.y = element_text(hjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(reverse = TRUE))
...
ggplot(coef_df, aes(x = estimate, y = predictor, color = model)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  theme_classic() +
  labs(title = "Func. Evenness Predictors at Different Scales",
       x = "Coefficient Estimate", y = "") +
  theme(axis.text.y = element_text(hjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(reverse = TRUE)) +
...
ggplot(coef_df, aes(x = estimate, y = predictor, color = model)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  theme_classic() +
  labs(title = "Func. Divergence Predictors at Different Scales",
       x = "Coefficient Estimate", y = "") +
  theme(axis.text.y = element_text(hjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(reverse = TRUE)) +
...
p1 <- ggplot(grid_int[grid_int$grid_size == 75,],
  ...
  labs(x = expression("0.75 km"^2* "grid cell ID"), y = "proportion of forest type partition", 
       fill = "partition", title = "") +
  theme_classic() +
  coord_flip() +
...
p2 <- ggplot(grid_int[grid_int$grid_size == 170,],
       aes(x = factor(grid_id), y = prop, fill = partition)) +
  geom_bar(stat = "identity") +
  labs(x = expression("1.7 km"^2* "grid cell ID"), y = "proportion of forest type partition", 
       fill = "partition", title = "") +
  theme_classic() +
  coord_flip() +
  scale_fill_manual(values = partition_colors)
...
adjCV_mets_long %>%
  filter(!metric %in% c("n.all", "shannon_ct", "div_even", "FRich", "FEven", "FDiver")) %>%
  filter(scale != "partition") %>%  # Exclude partition scale for individual points
  mutate(scale = factor(scale, levels = c("75 ha", "170 ha", "650 ha", "study area"))) %>%
  
  ggplot(aes(x = scale, y = value, group = metric)) +
  
  # Plot open grey circles for individual values
  geom_point(size = 2, shape = 1, color = "grey", position = position_jitter(width = 0.1), alpha = 0.6) +
  
  # Plot the mean trend line excluding the partition scale
  stat_summary(fun = mean, geom = "line", aes(color = "mean trend"), size = 1) +
  
  # Add the partition as a separate line segment at 170 ha
  geom_segment(aes(x = "170 ha", xend = "170 ha", 
                   y = mean(adjCV_mets_long$value[adjCV_mets_long$scale == "partition"], na.rm = TRUE), 
                   yend = mean(adjCV_mets_long$value[adjCV_mets_long$scale == "partition"], na.rm = TRUE)),
  ...
  # Set x-axis limits to remove partition space
  scale_x_discrete(labels = c("75 ha" = "0.75", 
                               "170 ha" = "1.7", 
                               "650 ha" = "6.5", 
                               "study area" = "26")) +
  
  # Use scale_color_manual if needed for color mapping
  scale_color_manual(values = c("mean trend" = "red"))


```


Again but without showing the full distribution
```{r}
# Split the metrics into two groups
metrics_top <- c(setdiff(ordered_metrics, c("n pts<2m", "tree h", "tree h sd")))
metrics_bottom <- c("n pts<2m", "tree h", "tree h sd")

# Ensure the correct ordering of metrics, with the bottom row metrics last
ordered_metrics <- c(metrics_top, metrics_bottom)

# Filter out the partition scale data from the main dataset
adjCV_mets_no_partition <- adjCV_mets_full %>%
  filter(scale != "partition") %>%
  mutate(metric_label = factor(metric_label, levels = ordered_metrics))

# Prepare data for partition mean CV values
partition_mean_cv <- adjCV_mets_full %>%
  filter(scale == "partition") %>%
  group_by(metric_label) %>%
  summarize(mean_value = mean(value, na.rm = TRUE), .groups = "drop")

# Calculate means for the trend line (excluding partition)
mean_adj_cv <- adjCV_mets_full %>%
  filter(scale != "partition") %>%
  group_by(metric_label, scale) %>%
  summarize(mean_value = mean(value, na.rm = TRUE), .groups = "drop")

# Plot
ggplot(adjCV_mets_no_partition, aes(x = scale, y = value, group = metric_label)) +
  geom_point(aes(color = metric_label), shape = 1, color = "grey", alpha = 0.7) +
  geom_line(data = mean_adj_cv, aes(y = mean_value), size = 1, color = "black") +
  geom_segment(data = partition_mean_cv,
               aes(x = 1.85, xend = 2.15, y = mean_value, yend = mean_value), 
               color = "forestgreen", size = 1.2) +
  facet_wrap(~ metric_label, scales = "free_y", ncol = 3) +
  labs(x = expression("spatial scale (in km"^2*")"), y = "adjusted coefficient of variation") +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("75 ha" = "0.75", 
                              "170 ha" = "1.7", 
