...
#ordering forest type by increasing elevation
m$habitat <- factor(m$habitat, levels = c("Peat Swamp", "Freshwater Swamp", "Alluvial Bench", 
                    ...
                    ordered = TRUE)
#ordering FT partitions by increasing elevation
...
#camera trap survey effort data
...
#terrestrial mammal traits data
...
Excluding arboreal mammals, dogs, and taxa not identified to species level (e.g., Muntiacus spp., Tragulus spp., and Unid civets).
...
### By FT partition - terrestrial mammals only
```{r}
#selecting terrestrial mammals, removing obs not identified to species
m_filtered <- m %>%
  filter(Terrestriality == 1 &
         !Species %in% unid_taxa)
...
Creating Shannon diversity metrics for terrestrial mammals for all partitions
...
### Diversity metrics by CT location/forest type
...
cmp3 <- ggplot(div_mets[div_mets$div_even > 0,], aes(x = habitat, y = div_even, color = habitat)) +
  ...
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
  labs(title = "", x = "", y = "species evenness") +
  coord_flip() +
  theme(legend.position = "none",
         axis.text.y = element_blank()) + 
  scale_color_manual(values = ft_colors)
...
fdp1 <- ggplot(fd_metrics_df, aes(x = habitat, y = FRich, color = habitat)) +
  ...
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
  labs(title = "", x = "", y = "functional richness") +
  coord_flip() +
...
fdp2 <- ggplot(fd_metrics_df, aes(x = habitat, y = FEven, color = habitat)) +
  ...
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
  labs(title = "", x = "", y = "functional evenness") +
  coord_flip() +
...
fdp3 <- ggplot(fd_metrics_df, aes(x = habitat, y = FDiver, color = habitat)) +
  ...
  geom_jitter(width = 0.1, size = 1) +
  theme_classic() +
  labs(title = "", x = "", y = "functional divergence") +
  coord_flip() +
  theme(axis.text.y = element_blank(),
        legend.position = "none") + 
  scale_color_manual(values = ft_colors)
...
#multiplot
cowplot::plot_grid(cmp1, cmp2, cmp3, fdp1, fdp3, fdp2,
                   nrow = 2, rel_widths = c(.47, .3, .3))
...
p6 <- ggplot(fd_metrics_df, aes(x = altitude, y = FDiver, color = habitat)) +
  ...
  theme_classic() +
  theme(legend.position = "none") + 
  labs(title = "", x = "elevation (m)", y = "functional divergence")

#dummy for legend extraction
p1_for_legend <- p1 + 
  geom_smooth(se = TRUE, aes(color = NULL), show.legend = FALSE) +
  guides(color = guide_legend(
    title = NULL, 
    reverse = TRUE,
    nrow = 1,                          
    override.aes = list(fill = NA, linetype = 0, shape = 16)
  )) +
...
#extract legend
legend <- cowplot::get_legend(p1_for_legend)

#create grid
panel_grid1 <- cowplot::plot_grid(p1 + theme(legend.position = "none"), p2, p3, 
                                  p4, p6, p5, nrow = 2)

#add legend
cowplot::plot_grid(panel_grid1, legend, ncol = 1, rel_heights = c(1, 0.1))

#ggsave("DivMets_by_elev_multiplot2.jpg", plot = last_plot(), width = 7.5, height = 5, dpi = 350, device = "jpeg")
```
...
Extracting Moran's eigenvector values to use in models
```{r}
...
colnames.list <- c("tree h mean", "tree h sd", "tree dbh mean", "basal area", "tree density", 
                   "stem volume", "near ground vegetation", "leaf area density", "gap volume", "vegetation density",
                   "vertical distribution", "vertical complexity", "species richness", "Shannon diversity",
...
Fitting global model then using {dredge} to create candidate models
```{r}
...
shannon_model <- glm(shannon_ct ~ 
                        ...
                        data = stand_mets)
#dredge
shan_models <- dredge(shannon_model, m.max = 3,
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc),
                      fixed = "eigenvectors.sc")
```
...
frich_model <- glm(FRich ~ 
              ...
              data = stand_mets_FD)
#dredge
frich_models <- dredge(frich_model, m.max = 3,
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc))
```
...
feven_model <- glm(FEven ~ 
              ...
              data = stand_mets_FD)
#dredge
feven_models <- dredge(feven_model, m.max = 3,
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc))
```
...
#dredge
shan_75_models <- dredge(sm_75, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(mean.tree.h.sc && vzrumple.sc))
```
...
#dredge
rich_170_models <- dredge(rm_170, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc))
```
...
#dredge
shan_170_models <- dredge(sm_170, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc))
```
...
#dredge
even_170_models <- dredge(em_170, m.max = 2, 
                      subset = !(basal.area.sc && stand.dens.sc))
```
...
#dredge
frich_170_models <- dredge(fr_170, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc))
```
...
#dredge
feven_170_models <- dredge(fe_170, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc)) 
```
...
#dredge
fdiver_170_models <- dredge(fd_170, m.max = 2,
                      subset = !(basal.area.sc && stand.dens.sc)) 
```
...
Cross-validation - iterating across all candidate models and averaging by outcome variable, weighted based on overall model weight
```{r}
library(boot)

get_weighted_metrics <- function(data, dredge_obj, K = 10, verbose = TRUE) {
  models  <- get.models(dredge_obj, subset = TRUE)
  weights <- dredge_obj$weight
  safe_cv <- safely(function(mod) cv.glm(data = data, glmfit = mod, K = K)$delta[2])
  
  #containers
  delta_vals <- numeric(length(models))
  mae_vals <- rmse_vals <- rep(NA, length(models))
  ...
  for (i in seq_along(models)) {
    ...
    #CV delta
    cv_res <- safe_cv(mod)
    delta_vals[i] <- ifelse(is.null(cv_res$result), NA, cv_res$result)
    
    #prediction metrics
    pred <- tryCatch(predict(mod, newdata = data, type = "response"), error = function(e) NA)
    obs  <- mod$y
    if (all(!is.na(pred))) {
      mae_vals[i]  <- mean(abs(obs - pred), na.rm = TRUE)
      rmse_vals[i] <- sqrt(mean((obs - pred)^2, na.rm = TRUE))
    }
  }
  
  ok <- !is.na(delta_vals)
  if (verbose) message(sum(!ok), " models failed (", sum(ok), " succeeded)")
  
  #weighted means
  weighted <- function(x) weighted.mean(x[ok], weights[ok], na.rm = TRUE)
  
  #metrics for all candidate models
  model_perf_table <- tibble(
    model_id = seq_along(models),
    AICc = dredge_obj$AICc[seq_along(models)],
    weight = weights,
    cv_delta = delta_vals,
    MAE = mae_vals,
    RMSE = rmse_vals)
  ...
  return(list(
    summary = tibble(
    mean_cv_delta = weighted(delta_vals),
    mean_MAE      = weighted(mae_vals),
    mean_RMSE     = weighted(rmse_vals)
  ),
  all_models = model_perf_table
  ))
}

#list the forest type partition candidate model objects
forest_models <- list(
  rich = rich_pt_models,
  shan = shan_pt_models,
  even = even_pt_models,
  fdiver = fdiver_pt_models,
  feven = feven_pt_models)

#list the 170 grid candidate model objects
grid_models <- list(
  rich = rich_170_models,
  shan = shan_170_models,
  even = even_170_models,
  fdiver = fdiver_170_models,
  feven = feven_170_models)


forest_metrics <- map(forest_models, ~ get_weighted_metrics(ct_div_mets_terr, .x, K = 11))
grid_metrics   <- map(grid_models,   ~ get_weighted_metrics(grid170_mets,   .x, K = 10))

names(forest_metrics) <- c("rich", "shan", "even", "fdiver", "feven")
names(grid_metrics)   <- c("rich", "shan", "even", "fdiver", "feven")

#extract summaries
forest_summary <- map_dfr(forest_metrics, "summary", .id = "metric") %>%
  mutate(model_type = "forest")

grid_summary <- map_dfr(grid_metrics, "summary", .id = "metric") %>%
  mutate(model_type = "grid")

combined_summary <- left_join(
  forest_summary %>% rename_with(~ paste0(.x, "_forest"), -c(metric, model_type)),
  grid_summary %>% rename_with(~ paste0(.x, "_grid"), -c(metric, model_type)),
  by = "metric") %>%
  dplyr::select(metric,
         starts_with("mean_cv_delta"),
         starts_with("mean_MAE"),
         starts_with("mean_RMSE"))

#write.csv(combined_summary, file = "CVmodelcompare_weighted.csv", row.names = FALSE)

#all candidate model results
forest_all <- map_dfr(forest_metrics, "all_models", .id = "metric") %>%
  mutate(model_type = "forest")

grid_all <- map_dfr(grid_metrics, "all_models", .id = "metric") %>%
  mutate(model_type = "grid")

all_models_combined <- bind_rows(forest_all, grid_all)
```

plotting
```{r}
model_colors <- c("forest" = "darkgreen", "grid" = "steelblue")

### ROW 1
#richness X MSE
p1 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "rich" & performance_metric == "cv_delta") %>%
  left_join(combined_summary %>%
      filter(metric == "rich") %>%
      dplyr::select(metric, mean_cv_delta_forest, mean_cv_delta_grid) %>%
      pivot_longer(cols = c(mean_cv_delta_forest, mean_cv_delta_grid),
                   names_to = "model_type",
                   values_to = "weighted_mean") %>%
      mutate(model_type = recode(model_type,
                                 mean_cv_delta_forest = "forest",
                                 mean_cv_delta_grid   = "grid")),
    by = c("metric", "model_type")) %>%
  ggplot(aes(x = model_type, y = value, fill = model_type)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
  geom_jitter(aes(color = model_type), width = 0.15, size = 2, alpha = 0.7) +
  geom_point(aes(y = weighted_mean), color = "orange", shape = 18, size = 4) +
  stat_summary(fun = mean, geom = "point", shape = 16, size = 2, color = "black") +
  labs(title = "Mean\nSquared Error", y = "Species Richness\n \nMSE", x = "") +
...
#richness X MAE
p2 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "rich" & performance_metric == "MAE") %>%
  left_join(combined_summary %>%
      filter(metric == "rich") %>%
      dplyr::select(metric, mean_MAE_forest, mean_MAE_grid) %>%
      pivot_longer(cols = c(mean_MAE_forest, mean_MAE_grid),
                   names_to = "model_type",
                   values_to = "weighted_mean") %>%
      mutate(model_type = recode(model_type,
                                 mean_MAE_forest = "forest",
                                 mean_MAE_grid   = "grid")),
    by = c("metric", "model_type")) %>%
  ggplot(aes(x = model_type, y = value, fill = model_type)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
...
#richness X RMSE
p3 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "rich" & performance_metric == "RMSE") %>%
  left_join(combined_summary %>%
      filter(metric == "rich") %>%
      dplyr::select(metric, mean_RMSE_forest, mean_RMSE_grid) %>%
      pivot_longer(cols = c(mean_RMSE_forest, mean_RMSE_grid),
                   names_to = "model_type",
                   values_to = "weighted_mean") %>%
      mutate(model_type = recode(model_type,
                                 mean_RMSE_forest = "forest",
                                 mean_RMSE_grid   = "grid")),
    by = c("metric", "model_type")) %>%
  ggplot(aes(x = model_type, y = value, fill = model_type)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
  ...
  geom_point(aes(y = weighted_mean), color = "orange", shape = 18, size = 4) +
  stat_summary(fun = mean, geom = "point", shape = 16, size = 2, color = "black") +
  labs(title = "Root Mean\nSquared Error", y = "RMSE", x = "") +
  scale_fill_manual(values = model_colors) +
  scale_color_manual(values = model_colors) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
### ROW 2
#shannon X MSE
p4 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "shan" & performance_metric == "cv_delta") %>%
  left_join(combined_summary %>%
      filter(metric == "shan") %>%
      dplyr::select(metric, mean_cv_delta_forest, mean_cv_delta_grid) %>%
      pivot_longer(cols = c(mean_cv_delta_forest, mean_cv_delta_grid),
...
#shannon X MAE
p5 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "shan" & performance_metric == "MAE") %>%
  left_join(combined_summary %>%
      filter(metric == "shan") %>%
      dplyr::select(metric, mean_MAE_forest, mean_MAE_grid) %>%
      pivot_longer(cols = c(mean_MAE_forest, mean_MAE_grid),
...
#shannon X RMSE
p6 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "shan" & performance_metric == "RMSE") %>%
  left_join(combined_summary %>%
      filter(metric == "shan") %>%
      dplyr::select(metric, mean_RMSE_forest, mean_RMSE_grid) %>%
      pivot_longer(cols = c(mean_RMSE_forest, mean_RMSE_grid),
...
### ROW 3
#even X MSE
p7 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "even" & performance_metric == "cv_delta") %>%
  left_join(combined_summary %>%
      filter(metric == "even") %>%
      dplyr::select(metric, mean_cv_delta_forest, mean_cv_delta_grid) %>%
      pivot_longer(cols = c(mean_cv_delta_forest, mean_cv_delta_grid),
...
#even X MAE
p8 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "even" & performance_metric == "MAE") %>%
  left_join(combined_summary %>%
      filter(metric == "even") %>%
      dplyr::select(metric, mean_MAE_forest, mean_MAE_grid) %>%
      pivot_longer(cols = c(mean_MAE_forest, mean_MAE_grid),
...
#even X RMSE
p9 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "even" & performance_metric == "RMSE") %>%
  left_join(combined_summary %>%
      filter(metric == "even") %>%
      dplyr::select(metric, mean_RMSE_forest, mean_RMSE_grid) %>%
      pivot_longer(cols = c(mean_RMSE_forest, mean_RMSE_grid),
...
### ROW 4
#fdiver X MSE
p10 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "fdiver" & performance_metric == "cv_delta") %>%
  left_join(combined_summary %>%
      filter(metric == "fdiver") %>%
      dplyr::select(metric, mean_cv_delta_forest, mean_cv_delta_grid) %>%
      pivot_longer(cols = c(mean_cv_delta_forest, mean_cv_delta_grid),
                   names_to = "model_type",
                   values_to = "weighted_mean") %>%
...
#fdiver X MAE
p11 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "fdiver" & performance_metric == "MAE") %>%
  left_join(combined_summary %>%
      filter(metric == "fdiver") %>%
      dplyr::select(metric, mean_MAE_forest, mean_MAE_grid) %>%
      pivot_longer(cols = c(mean_MAE_forest, mean_MAE_grid),
...
#fdiver X RMSE
p12 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "fdiver" & performance_metric == "RMSE") %>%
  left_join(combined_summary %>%
      filter(metric == "fdiver") %>%
      dplyr::select(metric, mean_RMSE_forest, mean_RMSE_grid) %>%
      pivot_longer(cols = c(mean_RMSE_forest, mean_RMSE_grid),
...
### ROW 5
#feven X MSE
p13 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "feven" & performance_metric == "cv_delta") %>%
  left_join(combined_summary %>%
      filter(metric == "feven") %>%
      dplyr::select(metric, mean_cv_delta_forest, mean_cv_delta_grid) %>%
      pivot_longer(cols = c(mean_cv_delta_forest, mean_cv_delta_grid),
...
#feven X MAE
p14 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "feven" & performance_metric == "MAE") %>%
  left_join(combined_summary %>%
      filter(metric == "feven") %>%
      dplyr::select(metric, mean_MAE_forest, mean_MAE_grid) %>%
      pivot_longer(cols = c(mean_MAE_forest, mean_MAE_grid),
  ...
  scale_color_manual(values = model_colors) +
  theme_minimal() +
  scale_x_discrete(labels = c("forest" = "partition", "grid" = "1.7 km² grid")) +
  theme(legend.position = "none")

#feven X RMSE
p15 <- all_models_combined %>%
  pivot_longer(cols = c(cv_delta, MAE, RMSE),
               names_to = "performance_metric",
               values_to = "value") %>%
  filter(metric == "feven" & performance_metric == "RMSE") %>%
  left_join(combined_summary %>%
      filter(metric == "feven") %>%
      dplyr::select(metric, mean_RMSE_forest, mean_RMSE_grid) %>%
      pivot_longer(cols = c(mean_RMSE_forest, mean_RMSE_grid),
...
#ggsave("PerfMetric_Multiplot.jpg", plot = last_plot(), width = 7, height = 8.5, dpi = 500, device = "jpeg")
```


Again for arithmetic means
```{r}
get_mean_metrics <- function(data, dredge_obj, K = 10, verbose = TRUE) {
  models  <- get.models(dredge_obj, subset = TRUE)
  safe_cv <- safely(function(mod) cv.glm(data = data, glmfit = mod, K = K)$delta[2])
  ...
  simple_mean <- function(x) mean(x[ok], na.rm = TRUE)
  ...
  return(list(
    summary = tibble(
      mean_cv_delta = simple_mean(delta_vals),
      mean_MAE      = simple_mean(mae_vals),
      mean_RMSE     = simple_mean(rmse_vals)
    ),
    all_models = model_perf_table
  ))
}

#apply forest and grid models
forest_metrics <- map(forest_models, ~ get_mean_metrics(ct_div_mets_terr, .x, K = 11))
grid_metrics   <- map(grid_models,   ~ get_mean_metrics(grid170_mets,   .x, K = 10))
...
#write.csv(combined_summary, file = "CVmodelcompare_NotWeighted.csv", row.names = FALSE)
```

Again for median values
```{r}
get_median_metrics <- function(data, dredge_obj, K = 10, verbose = TRUE) {
  models  <- get.models(dredge_obj, subset = TRUE)
  safe_cv <- safely(function(mod) cv.glm(data = data, glmfit = mod, K = K)$delta[2])
  ...
  #unweighted medians
  simple_median <- function(x) median(x[ok], na.rm = TRUE)
  ...
  return(list(
    summary = tibble(
      median_cv_delta = simple_median(delta_vals),
      median_MAE      = simple_median(mae_vals),
      median_RMSE     = simple_median(rmse_vals)
    ),
    all_models = model_perf_table
  ))
}

#apply to forest and grid models
forest_metrics <- map(forest_models, ~ get_median_metrics(ct_div_mets_terr, .x, K = 11))
grid_metrics   <- map(grid_models,   ~ get_median_metrics(grid170_mets,   .x, K = 10))
...
combined_summary <- left_join(
  ...
  dplyr::select(
    metric,
    starts_with("median_cv_delta"),
    starts_with("median_MAE"),
    starts_with("median_RMSE")
  )

#write.csv(combined_summary, file = "CVmodelcompare_Median.csv", row.names = FALSE)
```
...
### Gap volume
#ct scale - richness
cp3 <- ggpredict(get.models(rich_models, 1)[[1]], terms = "ClosedGapSpace.sc") %>%
  ggplot(aes(x = x, y = predicted)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "lightgreen", alpha = 0.3) +
  geom_line(color = "forestgreen", size = 1.2) +
  theme_classic() +
...
cp4 <- ggpredict(get.models(rich_models, 1)[[1]], terms = "vFRcanopy.sc") %>%
  ...
  geom_line(color = "forestgreen", size = 1.2) +
  theme_classic() +
  labs(x = "m³ (z)", y = "predicted species richness", title = "vegetation density\n(camera trap)") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ylim(0, 68)
...
cp5 <- ggpredict(get.models(rich_pt_models, 1)[[1]], terms = "vFRcanopy.sc") %>%
  ...
  theme_classic() +
  labs(x = "m³ (z)", y = "", title = "vegetation density\n(partition)") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ylim(0, 68)
...
cp6 <- ggpredict(get.models(rich_75_models, 1)[[1]], terms = "mean.dbh.sc") %>%
  ...
  theme_classic() +
  labs(x = "cm (z)", y = "", title = "tree dbh mean\n(0.75 km² grid)") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ylim(0, 68)
...
cp9 <- ggpredict(get.models(rich_170_models, 1)[[1]], terms = "vzrumple.sc") %>%
  ...
  geom_line(color = "steelblue", size = 1.2) +
  theme_classic() +
  labs(x = "vert. rumple (z)", y = "", title = "vertical complexity\n(1.7 km² grid)") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
#function to calculate small-sample adjusted coefficient of variation
...
Adjusted coefficient of variation increases as grid size increases across forest structure metrics.
...
adjCV_mets_long2 <- adjCV_mets2 %>%
  ...
  mutate(metric_label = recode(metric, 
                               ...
                               "zentropy" = "vertical distribution",
                               "lad.max" = "leaf area density",
                               "ClosedGapSpace" = "gap volume",
                               "vFRcanopy" = "vegetation density",
                               "sd.r" = "tree h sd",
                               "stem.vol" = "tree volume",
                               "mean.dbh" = "tree dbh mean",
                               "vzrumple" = "vertical complexity",
                               "stand.dens" = "tree density",
                               "basal.area" = "basal area",
                               "pts.below.2m" = "near ground vegetation"))
```

plot
```{r}
#set order
metrics_order2 <- c("tree h mean", "tree h sd", "tree dbh mean", "basal area", "tree density", 
                    "tree volume", "near ground vegetation", "leaf area density", "gap volume",  
                    "vegetation density", "vertical distribution", "vertical complexity")
...
adjCV_mets_long2 %>%
  ...
  geom_errorbar(data = partition_means,
                 aes(x = x, ymin = y, ymax = y),
                 color = "forestgreen", size = 1.2, width = 0.2,
                inherit.aes = FALSE) +
...
#ggsave("FS_by_scale_line-plot.jpg", plot = last_plot(), width = 6, height = 6, dpi = 500, device = "jpeg")
```

Again, showing the full distribution of values and setting matching y-axis scales
```{r}
adjCV_mets_long2 %>%
  mutate(scale_numeric = case_when(
    scale == "75 ha" ~ 0.75,
    ...
    scale == "partition" ~ 1.7)) %>%
  ggplot(aes(x = scale_numeric, y = value, group = metric)) +
  geom_jitter(aes(color = scale), width = 0.05, size = 1.5, alpha = 0.7) +
  stat_summary(fun = mean, geom = "line", aes(color = "mean trend"), size = 1) +
  stat_summary(fun = mean, geom = "point", aes(color = "mean trend"), size = 2) +
  geom_errorbar(data = partition_means, 
                aes(x = 1.7, ymin = y, ymax = y),
                color = "forestgreen", size = 1.2, width = 0.2,
                inherit.aes = FALSE) +
  facet_wrap(~ metric_label, ncol = 3, scales = "fixed") +
  labs(x = expression("spatial scale ("*km^2*")"), 
       y = "coefficient of variation") +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_x_log10(
    breaks = c(0.75, 1.7, 6.5, 26),
    labels = c("0.75", "1.7", "6.5", "26")
  ) +
  scale_color_manual(values = c(
    "75 ha" = "grey15",
...
#ggsave("FS_by_scale_dist.jpg", plot = last_plot(), width = 6, height = 6, dpi = 500, device = "jpeg")
```


Species table with observation counts and characteristic data used for calculating FD metrics
```{r}
