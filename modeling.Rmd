...
Determine which structure metrics best differentiate between forest types.
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
```{r}
stand_mets %>%
  dplyr::select(location, n.all, shannon_ct, div_even, max.height, sd.r, CRR.rho, pts.below.2m, stand.dens, basal.area, stem.vol, mean.tree.h, mean.dbh, zentropy, lad.max, rumple, vFRcanopy, vzrumple, ClosedGapSpace) %>%
  pivot_longer(cols = c(max.height, sd.r, CRR.rho, pts.below.2m, stand.dens, basal.area, stem.vol, mean.tree.h, mean.dbh, zentropy, lad.max, rumple, vFRcanopy, vzrumple, ClosedGapSpace), 
               names_to = "metric", 
               values_to = "value") %>%
  group_by(metric) %>%
...
Create table comparing correltaion coefficients for each metric (row) and each diversity metric (col)