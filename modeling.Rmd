...
summary_partition <- summarize_adj_cv(stand_mets, "partition")
...
adjCV_mets <- bind_rows(summary_75ha %>% mutate(scale = "75 ha"),
  ...
  separate(metric_stat, into = c("metric", "stat"), sep = "_mean_adj_cv") %>%
...
adjCV_mets %>%
  ...
  ggplot(aes(x = scale, y = value, group = metric, color = `75 < 170`)) +
