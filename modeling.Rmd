...
ggplot(coef_df_filter, aes(x = estimate, y = predictor, color = model)) +
  ...
  theme_classic() +
  labs(title = "Reliable Predictors from Averaged Models",
...
Basal area is highly correlated (\>0.8) with stand density, max height and mean tree h, 

```{r}
chart.Correlation(ct_div_mets_terr[,c("max.height.sc","sd.r.sc","CRR.rho.sc","pts.below.2m.sc","stand.dens.sc",
                                "basal.area.sc","stem.vol.sc","mean.tree.h.sc","mean.dbh.sc", "zentropy.sc",
                                "lad.max.sc","rumple.sc","vFRcanopy.sc","vzrumple.sc","ClosedGapSpace.sc")],
                  method = "spearman")
```
...
#### Comparing coefficient plot from averaged models
```{r}
## double check the fric by partition values, since the model coefficients seem to be 
```
