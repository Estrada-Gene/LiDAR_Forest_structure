...
#again for the functional metrics because they have one less observation point
...
spac_mets2 <- map_dfr(f_metrics_list, function(col) {
  metrics <- calculate_morans_i(stand_mets_func[[col]], weights)
  tibble(Metric = col,
         Moran_I = metrics$Moran_I,
         Expected_I = metrics$Expected_I,
         Variance_I = metrics$Variance,
...
#plot grid
plot(st_geometry(final_grid), col = "lightgrey", border = "white", main = "Grid with Survey Locations")
plot(st_geometry(data_sf), add = TRUE, col = "red", pch = 19)

#how many scanned locations in each grid cell
location_grid_assignment <- st_join(data_sf, final_grid, join = st_within)
table(location_grid_assignment$grid_id)
length(unique(location_grid_assignment$grid_id))
```


```{r}
# Drop geometry and keep only necessary columns
location_grid_assignment %>%
  st_drop_geometry() %>%
  dplyr::select(locationID, grid_id)
```


trying a different approach but doesn't work because of grid size
```{r}
# Convert the data to an sf object
survey_sf <- st_as_sf(stand_mets[,c("locationID", "longitude", "latitude")], 
                      coords = c("longitude", "latitude"), crs = 4326)

# Define a bounding box for the study area
bbox <- st_bbox(survey_sf)

# Create grid systems at different scales
grid_sizes <- c(45) 

# Function to create grid and assign locations
create_grid <- function(size, survey_sf) {
  # Create a grid of the specified size
  grid <- st_make_grid(survey_sf, cellsize = size, what = "polygons")
  grid_sf <- st_sf(grid_id = 1:length(grid), geometry = grid)
  
  # Assign survey locations to grid cells
  survey_grid <- st_join(survey_sf, grid_sf, join = st_within)
  
  # Ensure at least 10 grid cells contain survey locations
  grid_with_data <- survey_grid %>%
    group_by(grid_id) %>%
    filter(!is.na(grid_id)) %>%
    summarise(n = n()) %>%
    ungroup()
  
  if(nrow(grid_with_data) < 10) {
    stop("Grid size too large, fewer than 10 grid cells contain survey locations.")
  }
  
  return(survey_grid)
}

# Apply the function to each grid size
results <- lapply(grid_sizes, create_grid, survey_sf = survey_sf)

# Extract results for a specific grid size, e.g., 50m
result_50m <- results[[1]]  # Replace 1 with the index for the desired grid size

# Output table with survey locations and assigned grid boxes
output_table <- result_50m %>%
  st_drop_geometry() %>%
  select(id, grid_id)
