...
ggplot(pairs_df[!pairs_df$distance == 0,], 
  ...
  labs(x = "Geographic Distance (m)", y = "Structural Dissimilarity (Euclidean Distance)",
       color = "forest type", shape = "forest type") +
...
Comparison table showing intercepts and slopes of the linear models plotted above. 11/15 structure metrics show a lower intercept for the 'same partition' group compared to the 'different partition' group, while 9/15 show lower slope values for the 'same partition' group. 
```{r}
...
```{r}
hist(distance_df$distance[distance_df$distance > 0], las = 1, 
     main = "", xlab = "pwd (m)", breaks = 100)
```
...
One more large sized grid, at 310 ha, not to be modeled.  (65,5)
```{r} 
#for large sized grid (~170 ha)
initial_size <- 85 
min_cells <- 3
...
#how many scanned locations in each grid cell
grid310 <- st_join(data_sf, final_grid, join = st_within)
table(grid310$grid_id)
length(unique(grid310$grid_id))

#add grid designations to stand mets df
stand_mets <- grid310 %>%
  st_drop_geometry() %>%
  dplyr::select(locationID, grid_id) %>%
  rename(grid310 = grid_id) %>%
  left_join(stand_mets, .)

#adding grid to full CT list and then to mammal obs table (m)
grid310_cells <- st_join(ct_sf, final_grid, join = st_within)

m_filtered <- grid310_cells %>%
  st_drop_geometry() %>%
  dplyr::select(locationID, grid_id) %>%
  rename(grid.310 = grid_id) %>%
  left_join(m_filtered, .)
```

Again for the grid that I created in ArcGIS - 640 ha 2X2 
```{r}
...
coef_df_filter %>%
  filter(model %in% c("rich", "shan", "feve", "fdiv")) %>%
...
#specifying order to plot models
coef_df$model <- factor(coef_df$model, levels = rev(c("shan_ct","shan_75","shan_170","shan_pt")))
...
ggplot(coef_df, aes(x = estimate, y = factor(predictor, levels = rev(metrics_order)), color = model)) +
  ...
  geom_hline(yintercept = 3.5, color = "grey40") +
  theme_classic() +
  ...
  guides(color = guide_legend(reverse = TRUE)) +
  xlim(-2.8, 2.4) +
  annotate("text", x = 1.9, y = 13, label = "opposite direction\n of effects\n (eco. scales\n vs. grid scales)", color = "grey30", size = 3) +
  annotate("text", x = 1.9, y = 7, label = "same direction\n of effects", color = "grey30", size = 3)  +
  annotate("text", x = 1.9, y = 2, label = "mixed direction\n of effects", color = "grey30", size = 3) +
  scale_color_discrete(labels = rev(c("eco: camera trap", "grid: 75 ha",
                                      "grid: 170 ha", "eco: partition"))) +
...
#plot for 75 ha grid
ggplot(grid_int[grid_int$grid_size == 75,],
       aes(x = factor(grid_id), y = prop, fill = partition)) +
  geom_bar(stat = "identity") +
  labs(x = "Grid Cell ID", y = "Proportion of Forest Type Partition", 
       fill = "partition", title = "Partitions Within 75 ha Grid Cells") +
  theme_classic() +
  coord_flip()

#plot for 170 ha grid
ggplot(grid_int[grid_int$grid_size == 170,],
       aes(x = factor(grid_id), y = prop, fill = partition)) +
  geom_bar(stat = "identity") +
  labs(x = "Grid Cell ID", y = "Proportion of Forest Type Partition", 
       fill = "partition", title = "Partitions Within 170 ha Grid Cells") +
  theme_classic() +
  coord_flip()
```
...
Figure showing how average adjusted CV increases as grid size increases across the 15 forest structure metrics
```{r}
adjCV_mets %>%
  ...
  geom_text(data = . %>% filter(scale == "75 ha"), 
            aes(label = metric), 
            hjust = 1.2, 
            position = position_dodge(width = 0.2), 
            size = 2.5) +
  labs(x = "Grid Cell Area", y = "Avg. Coefficient of Variation", 
       title = "Comparison of Coefficient of Variation across Spatial Scales") +
  theme_minimal() +
