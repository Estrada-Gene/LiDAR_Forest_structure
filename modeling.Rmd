...
sm_pt1 <- glm(Shannon ~  
           ...
           data = ct_div_mets_terr)
#dredge
shan_pt_models <- dredge(sm_pt1, m.max = 2, subset = !(mean.tree.h.sc && vzrumple.sc))
...
CV iterating across all candidate models and averaging by outcome variable, weighted based on overall model weight
```{r}
...
forest_summary <- map_dfr(forest_metrics, bind_rows, .id = "metric") %>%
  mutate(model_type = "forest")

grid_summary <- map_dfr(grid_metrics, bind_rows, .id = "metric") %>%
...
Again without weighting averages
```{r}
...
forest_summary <- map_dfr(forest_metrics, bind_rows, .id = "metric") %>%
  rename_with(~ paste0(.x, "_forest"), -metric)

grid_summary <- map_dfr(grid_metrics, bind_rows, .id = "metric") %>%
  rename_with(~ paste0(.x, "_grid"), -metric)

# Combine for side-by-side comparison
combined_summary <- left_join(forest_summary, grid_summary, by = "metric") %>%
  dplyr::select(
    metric,
    mean_cv_delta_forest, mean_cv_delta_grid,
    mean_MAE_forest,      mean_MAE_grid,
    mean_RMSE_forest,     mean_RMSE_grid)
...
```{r}
#set order
metrics_order2 <- c("tree h mean", "tree h sd", "tree dbh", "basal area", "tree density", 
