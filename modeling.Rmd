...
rm1 <- glm(n.all ~  
           ...
           data = stand_mets)
#dredge
rich_models <- dredge(rm1, m.max = 3, extra = "deviance",
...
shannon_model <- glm(shannon_ct ~ 
                        ...
                        data = stand_mets)
#dredge
shan_models <- dredge(shannon_model, m.max = 3, extra = "deviance",
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc),
                      fixed = "eigenvectors.sc")
...
even_model <- glm(div_even ~ 
              ...
              data = stand_mets)
#dredge
even_models <- dredge(even_model, m.max = 3, extra = "deviance",
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc),
                      fixed = "eigenvectors.sc")
...
fdiver_model <- glm(FDiver ~ 
              ...
              data = stand_mets_FD)
#dredge
fdiver_models <- dredge(fdiver_model, m.max = 3, extra = "deviance",
                      subset = !(basal.area.sc && stand.dens.sc) && 
                               !(basal.area.sc && stem.vol.sc),
                      fixed = "eigenvectors.sc")
...
rm_75 <- glm(n.all ~ 
           ...
           family = Gamma(link = "log"), 
           offset = log(act.days),
           weights = n.scanned, #controlling for number of scanned locations/cell
           data = grid75_mets)
...
rich_170_models <- dredge(rm_170, m.max = 2, extra = "deviance",
                      subset = #!(basal.area.sc && stem.vol.sc) && 
                               !(basal.area.sc && stand.dens.sc)) 
                               #!(max.height.sc && mean.tree.h.sc) && 
                               #!(max.height.sc && vzrumple.sc) && 
...
#specifying order to plot models
coef_df$model <- factor(coef_df$model, levels = rev(c("Fric_ct","Fric_pt","Fric_75","Fric_170")))
...
```{r}
#get  top models
top_shanCT <- get.models(shan_models, 1)[[1]]
top_shanPT <- get.models(shan_pt_models, 1)[[1]]
top_shan75 <- get.models(shan_75_models, 1)[[1]]
top_shan170 <- get.models(shan_170_models, 1)[[1]]

#create null model
null_shanCT <- update(top_shanCT, . ~ 1)
null_shanPT <- update(top_shanPT, . ~ 1)
null_shan75 <- update(top_shan75, . ~ 1)
null_shan170 <- update(top_shan170, . ~ 1)

#compute pseudo R sqaured using McFadden's formula
1 - as.numeric(logLik(top_shanCT) / logLik(null_shanCT))
1 - as.numeric(logLik(top_shan75) / logLik(null_shan75))
1 - as.numeric(logLik(top_shan170) / logLik(null_shan170))
1 - as.numeric(logLik(top_shanPT) / logLik(null_shanPT))
```

Explained Deviance
```{r}
(deviance(null_shanCT) - deviance(top_shanCT)) / deviance(null_shanCT)
(deviance(null_shan75) - deviance(top_shan75)) / deviance(null_shan75)
(deviance(null_shan170) - deviance(top_shan170)) / deviance(null_shan170)
(deviance(null_shanPT) - deviance(top_shanPT)) / deviance(null_shanPT)
```
...
Deviance values are shown, again in parantheses to show the average deviance value to get to a cumulative model weight of at least 0.9
...
get.models(feven_pt_models, 1)[[1]] #deviance = 3.809 (3.85, n=2), wt = 0.625

get.models(rich_170_models, 1)[[1]] #deviance = 6.51, wt = 1
...
#ggsave("FS_by_scale_line-plot.jpg", plot = last_plot(), width = 7.5, height = 8, dpi = 500, device = "jpeg")
```
