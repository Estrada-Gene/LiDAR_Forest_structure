...
plot3 <- ggplot(stand_mets, aes(habitat, mean.dbh, color = habitat)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.1) + 
  labs(title = "tree dbh mean", x = "", y = "diameter (cm)") + 
  coord_flip() + 
  theme_classic() +
  theme(axis.text.y = element_blank(),
        plot.title = element_text(hjust = 0.5, size = title_size),
...
panel_grid1 <- cowplot::plot_grid(p1 + theme(legend.position = "none"), p2, p3, 
                                  p4, p5, p6, p7, p8, p9, p10, p11, p12, nrow = 4)

#add legend
cowplot::plot_grid(panel_grid1, legend, ncol = 1, rel_heights = c(1, 0.1))

#ggsave("FS_by_elev_multiplot2.jpg", plot = last_plot(), width = 7.5, height = 8, dpi = 500, device = "jpeg")
```

## All trees metrics

Using metrics from individual trees from all scans. Total of 2,010 trees detected by the FORTLS package.
```{r}
#tree h
...
t2 <- ggplot(tree_mets, aes(dbh)) +
  ...
  #                                                  "\nSD:", round(sd_dbh, 2)),
  #         hjust = 0.8, size = 4, color = "grey10") +
  labs(title = "tree dbh mean", x = "diameter (cm)", y = "") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
t2 <- ggplot(tree_mets, aes(x = altitude, y = dbh, color = habitat)) +
  ...
  geom_smooth(se = TRUE, aes(color = NULL)) +
  scale_color_manual(values = ft_colors) + 
  labs(title = "tree dbh mean", x = "elevation (m)", y = "diameter (cm)") + 
  theme_classic() +
...
#extract legend
legend <- cowplot::get_legend(t1_for_legend)

#create grid
panel_grid1 <- cowplot::plot_grid(t1 + theme(legend.position = "none"), t2, t3, t4, nrow = 1)

#add legend
cowplot::plot_grid(panel_grid1, legend, ncol = 1, rel_heights = c(1, 0.1))

#ggsave("tree_mets_elev2.jpg", plot = last_plot(), width = 7.5, height = 3, dpi = 500, device = "jpeg")
```
